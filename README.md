...
- 多端展示：Web 前端、飞书机器人和微信小程序实验端都支持基础聊天能力。
...
小程序目录位于 `miniprogram/`，通过 `wx.request` 调用现有 FastAPI `/api/chat`。该端目前作为实验端保留；如果不准备申请微信小程序正式发布所需的域名和 HTTPS 配置，可以优先使用 Web 端和飞书机器人端。
...
项目新增飞书机器人入口，适合部署在已有服务器上长期运行。用户在飞书私聊或群聊中向机器人发文字消息，飞书事件回调到 FastAPI，后端复用现有 Agent、RAG 和 tools 生成回答，再通过飞书消息接口回复。
...
1. 创建企业自建应用，启用机器人能力。
2. 在服务器 `.env` 中配置 `FEISHU_APP_ID`、`FEISHU_APP_SECRET`、`FEISHU_VERIFICATION_TOKEN`。
3. 在飞书开放平台的事件订阅中配置 Request URL：
...
- 飞书回复失败：服务端记录日志，用户不会看到 Python traceback。
